...
interface Estimate { id: string; estimateNo: string; title: string; customerId: string | null; projectId: string | null; taxRate: number; items: { name: string; description: string | null; quantity: number; unit: string; unitPrice: number; amount: number }[] }
...
function InvoiceNewPageInner() {
  ...
  const [estimateId, setEstimateId] = useState(searchParams.get("estimateId") ?? "");
  ...
  useEffect(() => {
    ...
    fetch("/api/estimates?status=APPROVED").then(r => r.json()).then(d => setEstimates(d.estimates ?? []));
  }, []);

  // 見積から読み込み
  useEffect(() => {
    const eid = searchParams.get("estimateId");
    if (!eid) return;
    fetch(`/api/estimates/${eid}`).then(r => r.json()).then(({ estimate }) => {
      if (!estimate) return;
      setEstimateId(estimate.id);
      setTitle(estimate.title);
      setTaxRate(estimate.taxRate);
      if (estimate.customerId) setCustomerId(estimate.customerId);
      if (estimate.projectId)  setProjectId(estimate.projectId);
      setItems(estimate.items.map((i: any) => ({
  ...
  function handleEstimateChange(eid: string) {
    setEstimateId(eid);
    if (!eid) return;
    ...
    if (est.customerId) setCustomerId(est.customerId);
    if (est.projectId)  setProjectId(est.projectId);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-900 font-medium">新規請求書作成</span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <Card padding="md" shadow="sm">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-900">承認済み見積から作成</p>
                <p className="text-xs text-gray-500 mt-0.5">見積を選ぶと明細・金額が自動入力されます</p>
              </div>
              <select
                className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white min-w-[200px]"
                value={estimateId}
                onChange={e => handleEstimateChange(e.target.value)}
              >
                <option value="">見積を選択…</option>
                {estimates.map(est => (
                  <option key={est.id} value={est.id}>{est.estimateNo}　{est.title}</option>
                ))}
              </select>
            </div>
          </Card>
        ...
        <Card padding="md" shadow="sm">
          ...
          <div className="space-y-4">
            <Input label="件名" required placeholder="例：〇〇邸新築工事　請求書" value={title} onChange={e => setTitle(e.target.value)} />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-gray-700">顧客（施主）</label>
                <select className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white" value={customerId} onChange={e => setCustomerId(e.target.value)}>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white" value={projectId} onChange={e => setProjectId(e.target.value)} required>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" rows={3} placeholder="支払条件・振込先・特記事項など" value={notes} onChange={e => setNotes(e.target.value)} />
            </div>
          </div>
        </Card>
        ...
        <Card padding="md" shadow="sm">
          ...
          <div className="mt-4 border-t border-gray-200 pt-4 space-y-1.5 text-sm text-right">
            ...
            <div className="flex justify-end gap-8 border-t border-gray-300 pt-2">
              ...
              <span className="font-bold text-emerald-700 text-lg w-32 text-right">¥{total.toLocaleString()}</span>
            </div>
          </div>
        </Card>

        <div className="flex justify-end gap-3">
          <Link href="/invoices"><Button type="button" variant="ghost" size="md">キャンセル</Button></Link>
          <Button type="submit" variant="primary" size="md" loading={submitting}>請求書を作成</Button>
        </div>
      </form>
    </div>
  );
